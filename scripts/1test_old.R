  ...
  { 
    ...
    n.sim = 100
    if (pilot == 1) {
      ntree = 100L; nmin = 6; nfold = 2           # tree parameters
    } else {
  ...
  # sapply(1:300, function(i) measure(result$w132$test$predicted[i,,drop=FALSE], Grid, t0 = 0, tau = tau, method = "int",
  #                                   surv.true = s.test[i,,drop=FALSE])["int.error"]) %>% as.numeric()
