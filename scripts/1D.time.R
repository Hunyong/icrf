...
set.seed(seed.no + 1); result$wH <- rf(split.rule = "Wilcoxon", quasihonesty = T, ERT = T, sampsize = ntrain * 0.95, replace = F); gc()
set.seed(seed.no + 1); result$wE <- rf(split.rule = "Wilcoxon", quasihonesty = F, ERT = T, sampsize = ntrain * 0.95, replace = F); gc()
set.seed(seed.no + 1); result$lH <- rf(split.rule = "logrank", quasihonesty = T, ERT = T, sampsize = ntrain * 0.95, replace = F); gc()
set.seed(seed.no + 1); result$lE <- rf(split.rule = "logrank", quasihonesty = F, ERT = T, sampsize = ntrain * 0.95, replace = F); gc()
