...
for i in "${arr[@]}"; do for j in {1..300}; do sbatch --mem=10000 --time=1:00:00 scripts/bash-icrf-sizeSim.sh $i $j 1 0 0; done; done
# For 1600 a larger memory is needed.
for i in "${arr2[@]}"; do for j in {1..300}; do sbatch --mem=30000 --time=2:00:00 scripts/bash-icrf-sizeSim.sh $i $j 1 0 0; done; done
