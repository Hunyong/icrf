# Avalanche data analysis - noise variables added.
...
out_path = "output/avalanche_noise"
if (!dir.exists(out_path)) dir.create(out_path)
fig_path = "figure/avalanche_noise"
...
# 0.4 Noise variables added
p.noise = 10
Sigma = diag(1, p.noise) * 0.8 + 0.2

set.seed(1)
x.noise = mvrnorm(n = aval.complete.n, mu = rep(0, p.noise), Sigma = Sigma)
nm.noise = colnames(x.noise) = paste0("X", 1:p.noise)
aval.noise = cbind(aval.complete, x.noise)
...
for (i in rng) {
  ...
  min.set <- aval.noise[samp1.list[[4]],]
  ...
  # train and test sets
  aval.train.list <- lapply(samp1.list, function(s) aval.noise[s, ])
  aval.test  <- aval.noise[samp2, ]
  ...
  for (j in 1:4) {
    ...
    set.seed(i + 100 * j)
    aval.icrf.H <- 
      icrf:::icrf.default(x = aval.train[, c("Country", "GroupActivity", "BurialDepth", nm.noise)], 
    ...
    cat("Cox models\n")
    ## 1.4.1 Cox
    frm = paste("Surv(log(L + 1), log(R + 1), type = 'interval2') ~ Country + GroupActivity + BurialDepth + ", 
                paste0(nm.noise, collapse = " + ")) %>% as.formula
    aval.cox <- icenReg::ic_sp(frm, model = "ph", data = aval.train)
    ...
    aval.pred <- lapply(list(aval.icrf.H, aval.icrf.E, aval.FuTR1, aval.FuTR2, aval.FuRF1, aval.FuRF2),
                        function(s) predict(s, newdata = aval.test[, c("Country", "GroupActivity", "BurialDepth", nm.noise)]))
